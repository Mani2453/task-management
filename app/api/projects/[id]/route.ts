...
export async function PUT(req: NextRequest, context: ProjectRouteContext) {
	try {
		...
		const { title, description, userId } = parse.data;
		if (!title) {
			return new Response(JSON.stringify({ error: 'Title and color are required.' }), { status: 400 });
		}
		...
		const result = await db.collection('projects').updateOne(
			{ _id: new ObjectId(id) }, // Changed: use the awaited id
			{ $set: { title, description, userId, updatedAt: new Date() } }
		);
