...
export async function PUT(req: Request, context: ProjectRouteContext) {
	try {
		...
		const { title, description, status, priority, dueDate, userId, projectId } = parse.data;
		...
		const result = await db.collection('tasks').updateOne(
			{ _id: new ObjectId(id) },
			{ $set: { title, description, status, priority, dueDate, userId, projectId, updatedAt: new Date() } }
		);
